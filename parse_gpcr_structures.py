            ...
            if is_aa(residue, standard=True):
                for atom_res in residue:
                    if atom_lig - atom_res < threshold:
                        ...
                        break
    return binding_res
# Add this function to your scripts/modeling/parse_gpcr_structures.py file
# Make sure to import numpy at the top if not already present: import numpy as np

def process_structure(uniprot_id: str, pdb_id: str = None, af_path: str = None, chain_id: str = 'A', ligand_id: str = None, is_apo: bool = False):
    """
    Processes a PDB or AF2 structure, extracts relevant data including full protein sequence,
    UniProt-mapped residue indices, and Calpha coordinates.

    Args:
        uniprot_id (str): UniProt ID of the protein.
        pdb_id (str, optional): PDB ID of the structure.
        af_path (str, optional): Path to local AlphaFold2 PDB file.
        chain_id (str, optional): Specific chain ID to process. Defaults to 'A'.
        ligand_id (str, optional): Ligand ID if processing a ligand-bound structure.
        is_apo (bool, optional): True if structure is Apo (for logging/context).

    Returns:
        dict: A dictionary containing:
            - 'uni_idxs' (np.ndarray): NumPy array of UniProt indices for mapped residues.
            - 'coords' (np.ndarray): Corresponding Calpha coordinates.
            - 'full_sequence' (str): Full protein sequence of the processed chain.
            - 'pdb_id' (str): The PDB_ID used (or 'AF2' if from AlphaFold).
        Returns None if processing fails or essential data is missing.
    """
    structure = None
    file_type = "cif" 
    source_id = pdb_id # To track if it's PDB_ID or 'AF2'

    if pdb_id:
        file_path = download_mmcif(pdb_id) # Try mmCIF first
        if not file_path:
            file_path = download_pdb(pdb_id) # Fallback to PDB
            file_type = "pdb"
        if file_path:
            structure = parse_structure(pdb_id, file_path, file_type)
    elif af_path and os.path.exists(af_path):
        structure = parse_structure(uniprot_id, af_path, "pdb") # AF2 files are PDB format
        source_id = "AF2" # Mark source as AF2
    
    if structure is None:
        print(f"[ERROR] Could not load structure for {uniprot_id} from {pdb_id or af_path}.")
        return None
    ...
    for model in structure:
        if chain_id in model:
            ...
            break
    if not target_chain:
        print(f"[ERROR] Chain {chain_id} not found in structure for {uniprot_id} ({source_id}).")
        return None

    # Extract full sequence from the chain
    polypep_builder = Polypeptide.PPBuilder()
    peptides = polypep_builder.build_peptides(target_chain)
    full_sequence_of_chain = "".join(str(p.get_sequence()) for p in peptides)
    
    if not full_sequence_of_chain:
        print(f"[ERROR] Could not extract sequence from chain {chain_id} for {uniprot_id} ({source_id}).")
        return None

    # Fetch UniProt canonical sequence for alignment (needed for map_pdb_res_to_uniprot)
    uni_seq = fetch_uniprot_sequence(uniprot_id)
    if not uni_seq:
        print(f"[ERROR] Could not fetch UniProt sequence for {uniprot_id}.")
        return None

    # Perform local alignment to map PDB residues to UniProt indices
    aln_result = local_align_smith_waterman(full_sequence_of_chain, uni_seq)
    
    # Get list of all amino acid residues (Bio.PDB.Residue objects) in the chain for mapping
    pdb_res_list = [res for res in target_chain if is_aa(res, standard=True)]

    # Map PDB residue objects to UniProt indices
    res_map = map_pdb_res_to_uniprot(pdb_res_list, aln_result["aligned_pdb_seq"], aln_result["aligned_uni_seq"])

    # Extract Calpha coordinates and corresponding UniProt indices for all *mapped* residues
    uni_idxs = []
    coords = []
    for pdb_res in pdb_res_list:
        if pdb_res in res_map and 'CA' in pdb_res: # Ensure residue is mapped and has a Calpha atom
            uni_idxs.append(res_map[pdb_res])
            coords.append(pdb_res['CA'].get_coord())
            
    if not uni_idxs:
        print(f"[ERROR] No Calpha coordinates found or mapped for {uniprot_id} ({source_id}). This might indicate alignment issues or missing Calpha atoms.")
        return None

    return {
        'uni_idxs': np.array(uni_idxs, dtype=int), # UniProt indices of all aligned residues with coords
        'coords': np.array(coords, dtype=np.float32), # Calpha coordinates for these residues
        'full_sequence': full_sequence_of_chain, # Full sequence extracted from the PDB chain
        'pdb_id': source_id # 'PDB_ID' or 'AF2'
    }
...
def process_binding_residues(rep_chain_df, rep_ligand_df, dist_thresholds=(4.0, 6.0, 8.0)):
    ...
    df_merged = pd.merge(df_chain, rep_ligand_df, how="inner",
                         left_on=["UniProt_ID", "PDB_ID","chain_id"],
                         right_on=["UNIPROT_AC", "PDB_ID","chain_id"])
    results = []
    for _, row in tqdm(df_merged.iterrows(),total=len(df_merged),desc = "PDB→binding‐res"):
...
def map_binding_residues_to_uniprot(binding_res_df):
    ...
    records = []
    for _, row in tqdm(binding_res_df.iterrows(),total = len(binding_res_df),desc="Binding res --> uniprot"):
        uniprot_id = row["UniProt_ID"]
        pdb_id = row["PDB_ID"]
        chain_id = row["chain_id"]
        ligand_id = row["ligand_id"]
...
def union_binding_residues_by_gpcr(mapped_df):
    """Generate GPCR_Binding_Residues_UniProt.csv, with a progress bar."""
    mapped_df["res_set"] = mapped_df["binding_res_uniprot"].apply(
        lambda x: set(ast.literal_eval(x)) if isinstance(x, str) else set(x)
    )
    records = []
    # wrap the groupby in tqdm to show progress over UniProt IDs
    for uni_ac, grp in tqdm(mapped_df.groupby("UniProt_ID"),
                            desc="Union binding residues", 
                            total=mapped_df["UniProt_ID"].nunique()):
...
def process_gpcr_structures(df_pdb_info, dist_thresholds=(4.0, 6.0, 8.0)):
    ...
    df_chains = pd.read_csv(os.path.join(OUTPUT_DIR, "Rep_GPCR_chain.csv"))
    
    print("Processing ligands...")
    df_ligands = pd.read_csv(os.path.join(OUTPUT_DIR, "Rep_Ligand.csv"))
    
    print("Processing binding residues...")
    df_binding_res = pd.read_csv(os.path.join(OUTPUT_DIR, "PDB_Binding_Res.csv"))
    
    print("Mapping binding residues to UniProt...")
    df_mapped_res = pd.read_csv(os.path.join(OUTPUT_DIR, "C:\\Users\\user\\HJ-DR-GPCRact\\Output\\Binding_Residue\\PDB_Binding_Residues_UniProt.csv"))
    ...
    for _, row in df_pdb_info.iterrows():
        ...
        if not binding_row.empty:
            raw = binding_row.iloc[0]["Binding_Residues"]
            binding_res = ast.literal_eval(raw) if isinstance(raw, str) else raw
