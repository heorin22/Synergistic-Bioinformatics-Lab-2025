...
# In scripts/modeling/utils_structure.py
# Change the import to explicitly list needed functions using a relative path
from .parse_gpcr_structures import (
    download_mmcif, download_pdb, download_alphafold_structure,
    parse_structure, fetch_uniprot_sequence, local_align_smith_waterman,
    map_pdb_res_to_uniprot, extract_resolution_from_cif # Add extract_resolution_from_cif if it's there
)
# Note: Bio.PDB imports (like Polypeptide, is_aa, MMCIFParser, PDBParser)
# should remain at the top of utils_structure.py if used directly there.
import numpy as np
...
def compute_rmsd_and_disp(coords1: np.ndarray, coords2: np.ndarray) -> tuple:
    """
    Computes the overall RMSD and per-residue displacements (distances)
    between two sets of C-alpha coordinates after optimal superposition.

    Args:
        coords1 (np.ndarray): NumPy array of C-alpha coordinates for the first structure (reference).
                              Shape: (N, 3), where N is the number of common residues.
        coords2 (np.ndarray): NumPy array of C-alpha coordinates for the second structure (moving).
                              Shape: (N, 3), where N is the number of common residues.

    Returns:
        tuple: (overall_rmsd, per_residue_displacements, transformed_coords2)
            - overall_rmsd (float): The root-mean-square deviation after superposition.
            - per_residue_displacements (np.ndarray): A 1D array of displacements (distances)
                                                      for each corresponding residue.
            - transformed_coords2 (np.ndarray): The coordinates of the second structure
                                                after optimal rotation and translation
                                                to superimpose onto the first.
    """
    if coords1.shape != coords2.shape or coords1.shape[0] == 0:
        raise ValueError("Input coordinate arrays must have the same non-zero shape (N, 3).")
    
    # Create dummy Bio.PDB.Atom objects from the NumPy arrays
    # This is a workaround because Superimposer.set_atoms expects Atom objects,
    # even when given what it might internally convert to coordinates.
    fixed_atoms = [BioPDBAtom(f"CA{i}", coords1[i], 0, 0, 'C', f"CA{i}", i, 'C') for i in range(coords1.shape[0])]
    moving_atoms = [BioPDBAtom(f"CA{i}", coords2[i], 0, 0, 'C', f"CA{i}", i, 'C') for i in range(coords2.shape[0])]

    # Initialize the Superimposer
    sup = Superimposer()
    
    # Set the atoms to be superimposed: (fixed_atoms, moving_atoms)
    sup.set_atoms(fixed_atoms, moving_atoms) # Now passing lists of BioPDBAtom objects
    
    # Get the rotation and translation
    rotation_matrix = sup.rotran[0]
    translation_vector = sup.rotran[1]
    
    # Apply the transformation to the original coords2 NumPy array
    transformed_coords2 = np.dot(coords2, rotation_matrix) + translation_vector
    
    # Calculate per-residue displacements
    per_residue_displacements = np.linalg.norm(coords1 - transformed_coords2, axis=1)
    
    # The overall RMSD is also available from the Superimposer
    overall_rmsd = sup.rms

    return overall_rmsd, per_residue_displacements, transformed_coords2
...
def main():
    ...
    parser.add_argument(
        "--binding_residues_csv", type=str, default="data/GPCR_Binding_Residues_UniProt.csv",
        help="Path to the CSV file with binding site residues for each UniProt ID.\n(Default: data/GPCR_Binding_Residues_UniProt.csv)"
    )
    parser.add_argument(
